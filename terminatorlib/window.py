...
#        self.set_property('allow-shrink', True)  # FIXME FOR GTK3, or do we need this actually?
    ...
    def on_destroy_event(self, widget, data=None):
        ...
        dbg('destroying self')
        if not self.isDestroyed:  # Защита от повторного вызова
            ...
            # Удаляем вызов self.destroy(), так как он может вызвать рекурсию
            # self.destroy()
            # Также удаляем del(self), поскольку это может создать проблемы с памятью
            # del(self)
